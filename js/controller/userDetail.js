							...
							firebase.database().ref(userHistoryRef).set(true).then(
  								function(s) {
  									...
  									facturaRequest.$loaded().then(function(){
  										var update = {
  											pay: false,
  											usersDesc: facturaRequest.usersDesc ? (!isDescActive ? facturaRequest.usersDesc + 1 : facturaRequest.usersDesc) : (!isDescActive ? 1 : 0),
  											usersFree: facturaRequest.usersFree ? (isDescActive ? facturaRequest.usersFree + 1 : facturaRequest.usersFree) : (isDescActive ? 1 : 0)
  										};
